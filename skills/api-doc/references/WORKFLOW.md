...
生成文档前，汇总所有接口分析结果，决定对象和枚举的定义位置：

**非泛型对象归属判定：**
- 统计每个对象被哪些接口引用
- 被 2+ 接口引用 → 标记为公共对象，后续放入「公共对象定义」章节，各接口通过锚点引用
- 仅被 1 个接口引用 → 标记为内联对象，定义在该接口内部

**泛型对象（如 PageInfo\<T\>）：** 始终内联到每个接口中，因泛型参数不同导致内部结构不一致

**枚举汇总：** 收集所有接口涉及的枚举类，统一放入「枚举定义汇总」章节
