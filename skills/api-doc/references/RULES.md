...
- **泛型对象**（如 PageInfo\<T\>）→ 始终内联到每个接口中，因泛型参数不同导致内部结构不一致，不得提取为公共对象
- **非泛型对象被 2 个及以上接口引用** → 抽取到「公共对象定义」章节，各接口通过锚点引用
- **非泛型对象仅被 1 个接口使用** → 内联定义在该接口的请求体或响应结构中
- **嵌套对象仅被 1 个父对象引用** → 内联定义在该父对象的字段表格下方，不提取为公共对象

### 公共对象引用方式

在接口的响应结构中，用简短说明 + 锚点引用：

```markdown
#### data 对象

见 [OpenApiServiceRes 对象](#openapiserviceres-对象)
```

### 内联对象定义方式

直接在接口的请求体或响应结构下方用 `####` 定义：
