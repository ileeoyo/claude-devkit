...
### `/output-polish` - 输出友好整理

默认把上一条 AI 输出交给 `claude` 非交互命令做信息保真的结构整理；如果用户显式传入内容，则整理传入内容，让分析内容更适合人类阅读，同时不新增、修改或删除原内容信息。

**使用方式：**

```bash
/output-polish 整理一下上面的输出
/output-polish 用 GLM-5.1 把上面的分析内容重新整理得好读一点
```
